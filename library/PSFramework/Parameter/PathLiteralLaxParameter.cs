...
namespace PSFramework.Parameter
{
    ...
    /// </summary>
    public class PathLiteralLaxParameter : PathFileSystemParameterBase
    {
        #region Constructors
        /// <summary>
        ...
        public PathLiteralLaxParameter(string Path)
        {
            InputObject = Path;
            foreach (string filePath in ResolveFileSystemPath(Path, true, true, false, false))
                AddEx(filePath);
        }
        /// <summary>
        ...
        public PathLiteralLaxParameter(string[] Path)
        {
            ...
            foreach (string entry in Path)
                foreach (string filePath in ResolveFileSystemPath(entry, true, true, false, false))
                    AddEx(filePath);
        }
        /// <summary>
        ...
        public PathLiteralLaxParameter(FileInfo File)
        {
            InputObject = File;
            if (!File.Exists)
        ...
        /// <exception cref="ArgumentException"></exception>
        public PathLiteralLaxParameter(DirectoryInfo Directory)
        {
        ...
        public PathLiteralLaxParameter(Uri Uri)
        ...
        public PathLiteralLaxParameter(object Input)
        {
            if (Input == null)
