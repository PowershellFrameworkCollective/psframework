...
#>
	...
	process
	{
		...
		$pipeline = [pscustomobject]@{
			...
			OutputAssigned = $false
			OutputAssignedTo = $null
			InputFromVar = $false
			InputVariable = ""
			InputDirect = $false
			InputValue = $null
		...
		foreach ($command in $commands)
		{
			...
			$pipeline.Commands += [PSCustomObject]@{
				Index   = $index
				Command = Get-PrivateProperty -Object $command -Name CommandInfo
				Invocation = $commandItem.MyInvocation
				InvocationName = $commandItem.MyInvocation.InvocationName
			}
			$index++
		}
