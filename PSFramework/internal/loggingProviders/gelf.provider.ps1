...
$configuration_Settings = {
	...
	Set-PSFConfig -Module LoggingProvider -Name 'GELF.ExcludeTags' -Value @() -Initialize -Validation "stringarray" -Handler { if ([PSFramework.Logging.ProviderHost]::Providers['gelf']) { [PSFramework.Logging.ProviderHost]::Providers['gelf'].ExcludeTags = $args[0] } } -Description "Tag blacklist. Messages with these tags will not be logged"
}
